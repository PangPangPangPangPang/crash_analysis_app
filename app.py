...
class CustomPanel(wx.Panel):
    ...
    def initSubView(self):
        ...
        self.list = wx.ListBox(self, size=(200, 200))
        if self.item is not None:
            self.list.Bind(wx.EVT_LISTBOX, self.item.onSelect)
        vBox.Add(self.list, flag=wx.TOP, border=10)

        self.SetSizer(vBox)

class CustomFrame(wx.Frame):
    def __init__(self, parent, title):
        super(CustomFrame, self).__init__(parent, title=title, size=(400, 550))
    ...
    def initUI(self):
        ...
        vSizer.Add(self.chooseButton)
        vSizer.Add(self.beginButton)
